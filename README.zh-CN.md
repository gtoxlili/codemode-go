...
`ConflictKeys` 是混合批量得以安全执行的前提:binding 声明每次调用触及哪个资源,于是读取不同文件依然并行,而对同一文件的写入和读取会串行。只有 runtime 知道这件事,引擎没有工具的参数 schema。具体规则见[调度](#调度)。
...
各家实现的差别在于代码执行**用来做什么**。不少实现把它作为触达工具的主路径:模型只剩一个 `execute_code`,其余工具都从它里面走。本项目刻意是增量的:它是嵌入自有 runtime 的库,你的工具表仍是唯一真相源,程序工具只负责把它投影出去。模型保留原有的每一个工具,同时多了一条触达同一批能力的路径。详见[横向对比](#横向对比)。
...
**优势。** 纯 Go、单进程,程序与工具之间不存在协议边界,也不对 agent 循环提出任何要求。它对资源冲突做了显式建模,而不是把每个并发调用都当作互相独立:`Mutating` 和 `ConflictKeys` 是宿主用以说明"这次调用会写入文件 X"的手段,而这件事只有宿主知道。引擎之外还包含:system prompt 段落、面向模型自修正编写的失败分类,以及每次子调用前后的钩子。

启动一次运行的开销是一个 goja VM,微秒级。所以一个只有两次调用的任务不该走程序,原因是写这段程序要花的 token,不是运行时开销。
...
**串行需要两边都提供 key。** 没有 key 的调用不与任何东西构成冲突,因此只给写入方声明换不到任何东西:对同一文件的写入和读取照样并发,除非读取方也指名了那个文件。凡是触及某个资源的工具都要声明,不只是会改动它的那些。
...
那种形态确实存在于别处,并且有一个值得了解的结构性限制:MCP 是不对称的。server 侧暴露 tools,client 侧暴露 roots / sampling / elicitation,协议中没有任何机制允许 server 反向调用 client。因此网关无法触达 Claude Code 的 Read、Edit 或 Bash,它能投影的只有自己连接得到的那些能力。这一条是任何部署方式都绕不开的,也正是网关不适合"把 runtime 自身的原语变为可编程"这个目标的原因。若一批 MCP 工具就是全部问题所在,网关是个合理的答案。
