...
// budget and the memory trip line are sampled every 50ms and enforced with
// goja's Interrupt. Sampling sets how precisely they fire, not how soon: an
// allocation bomb crosses the memory line almost at once, while a loop that
// only computes runs until its compute budget is gone. Both are what a model
// writes by accident. Neither makes the VM safe against an adversary who
// controls the program and is willing to spend real effort on it.
package codemode
